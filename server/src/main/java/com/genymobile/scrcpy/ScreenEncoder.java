...
import com.genymobile.scrcpy.rtmp.RtmpMessageUtils;
import com.genymobile.scrcpy.rtmp.RtmpStreamClient;
...
public class ScreenEncoder implements Device.RotationListener {
    ...
    private void tryOpenRtmpStreaming() {
        new Thread(() -> {
            boolean connected = streamClient.connect(10);
            if (connected) {
        ...
        try {
            do {
                ...
                if (rtmpServerUrl != null) {
                    streamClient = new RtmpStreamClient(60, videoRect.width(), videoRect.height(), rtmpServerUrl);
    ...
    private void handleVideoFormatChanged(MediaFormat newFormat) {
        streamClient.putMessage(RtmpMessageUtils.parseVideoFormatChanged(newFormat));
    }

    private void handleVideoDataArrived(MediaCodec.BufferInfo bufferInfo, ByteBuffer codecBuffer) {
        streamClient.putMessage(RtmpMessageUtils.parseFromSystemMediaVideoCodec(bufferInfo, codecBuffer));
    }
    ...
    private boolean encode(MediaCodec codec, FileDescriptor fd) throws IOException {
        ...
        while (!consumeRotationChange() && !eof) {
            ...
            try {
                ...
                if (isRtmpStreamingEnabled() && outputBufferId == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                    ...
                    if (isRtmpStreamingEnabled() && bufferInfo.size != 0 && bufferInfo.flags != MediaCodec.BUFFER_FLAG_CODEC_CONFIG) {
                        codecBuffer.rewind();
