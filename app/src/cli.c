...
#define ENABLE_RTMP_STREAMING 1020
...
bool scrcpy_parse_args(struct scrcpy_cli_args *args, int argc, char *argv[])
{
    static const struct option long_options[] = {
        ...
        {"rtmp-server-url", required_argument,NULL,ENABLE_RTMP_STREAMING},
        {NULL, 0, NULL, 0},
    };
                            ...
                            long_options, NULL)) != -1)
    {
        ...
        case OPT_FORCE_ADB_FORWARD:
            ...
            break;
        case ENABLE_RTMP_STREAMING:
            LOGI("Enable RTMP Support! %s",optarg);
            opts->rtmp_server_url = SDL_strdup(optarg);
            break;
        default:
    ...
    if(opts->rtmp_server_url && strncmp(opts->rtmp_server_url,RTMP_URL_PREFIX,sizeof(RTMP_URL_PREFIX)-1))
    {
        LOGE("RTMP server uurl has an invalid scheme prefix.");
        return false;
    }
    return true;
}
